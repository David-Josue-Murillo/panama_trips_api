...
public interface AuditLogRepository extends JpaRepository<AuditLog, Integer> {
        ...
        List<AuditLog> findByIpAddressAndActionTimestampBetween(String ipAddress, LocalDateTime start,
                        LocalDateTime end);
        ...
        @Query("SELECT a FROM AuditLog a WHERE a.oldValues IS NULL")
        List<AuditLog> findByOldValuesIsNullOrEmpty();

        @Query("SELECT a FROM AuditLog a WHERE a.newValues IS NULL")
        List<AuditLog> findByNewValuesIsNullOrEmpty();

        @Query("SELECT a FROM AuditLog a WHERE a.oldValues IS NULL AND a.newValues IS NULL")
        List<AuditLog> findByBothValuesNullOrEmpty();

        @Query("SELECT a FROM AuditLog a WHERE a.oldValues IS NOT NULL OR a.newValues IS NOT NULL")
